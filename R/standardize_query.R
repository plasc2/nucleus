...
standardize_query <- function(query) {
  ...
  if (is.na(suppressWarnings(as.integer(query))) == FALSE) {
    if (nchar(query) < 2) {
      ...
      if (query %in% nucleus::k.states$GEOID) {
        return(c(NAME = nucleus::k.states$NAME[nucleus::k.states$GEOID == query],
                 GEOGRAPHY = "State",
      ...
      if (substr(query, 0, 2) %in% nucleus::k.states$GEOID) {
        t.reference.2 <- suppressMessages(tigris::counties(state = nucleus::k.states$SHORT[which(nucleus::k.states$GEOID == substr(query, 0, 2))], year = 2020, cb = TRUE))
      }
        ...
        if (query %in% t.reference.1$GEOID & query %in% t.reference.2$GEOID) {
          return(list(
            ...
            c(NAME = t.reference.2$NAMELSAD[which(t.reference.2$GEOID == query)],
              GEOGRAPHY = "County",
              LEVEL = nucleus::k.geographies$LEVEL[nucleus::k.geographies$GEOGRAPHY == "County"],
              GEOID = query,
              STATE = nucleus::k.states$SHORT[which(nucleus::k.states$GEOID == substr(query, 0, 2))])
          ))
      ...
      if (substr(query, 0, 2) %in% nucleus::k.states$GEOID) {
        t.reference.1 <- suppressMessages(tigris::zctas(state = nucleus::k.states$SHORT[which(nucleus::k.states$GEOID == substr(query, 0, 2))], year = 2010))
        t.reference.2 <- suppressMessages(tigris::school_districts(state = nucleus::k.states$SHORT[which(nucleus::k.states$GEOID == substr(query, 0, 2))], year = 2020, cb = TRUE))
        t.reference.3 <- suppressMessages(tigris::places(state = nucleus::k.states$SHORT[which(nucleus::k.states$GEOID == substr(query, 0, 2))], year = 2020, cb = TRUE))
      } else {
      ...
      if (query %in% t.reference.1$GEOID & query %in% t.reference.2$GEOID & query %in% t.reference.3$GEOID) {
        return(list(
          c(NAME = t.reference.1$ZCTA5CE10[t.reference.1$GEOID == query],
            GEOGRAPHY = "ZIP Code",
            LEVEL = nucleus::k.geographies$LEVEL[nucleus::k.geographies$GEOGRAPHY == "ZIP Code"],
            GEOID = query,
            STATE = nucleus::k.states$SHORT[nucleus::k.states$GEOID == substr(query, 0, 2)]),
          c(NAME = t.reference.2$NAMELSAD[t.reference.2$GEOID == query],
            GEOGRAPHY = "School District",
            LEVEL = nucleus::k.geographies$LEVEL[nucleus::k.geographies$GEOGRAPHY == "School District"],
            GEOID = query,
            STATE = nucleus::k.states$SHORT[nucleus::k.states$GEOID == substr(query, 0, 2)]),
      ...
      if (substr(query, 0, 2) %in% nucleus::k.states$GEOID) {
        t.reference <- suppressMessages(tigris::tracts(state = nucleus::k.states$SHORT[nucleus::k.states$GEOID == substr(query, 0, 2)], year = 2020, cb = TRUE))
        if (query %in% t.reference$GEOID) {
          return(c(NAME = t.reference$NAMELSAD[t.reference$GEOID == query],
                   GEOGRAPHY = "Tract",
                   LEVEL = nucleus::k.geographies$LEVEL[nucleus::k.geographies$GEOGRAPHY == "Tract"],
                   GEOID = query,
                   STATE = nucleus::k.states$SHORT[nucleus::k.states$GEOID == substr(query, 0, 2)]))
        } else {
      ...
      if (substr(query, 0, 2) %in% nucleus::k.states$GEOID) {
        t.reference <- suppressMessages(tigris::block_groups(state = nucleus::k.states$SHORT[nucleus::k.states$GEOID == substr(query, 0, 2)], year = 2020, cb = TRUE))
        if (query %in% t.reference$GEOID) {
          return(c(NAME = t.reference$NAMELSAD[t.reference$GEOID == query],
                   GEOGRAPHY = "Block Group",
                   LEVEL = nucleus::k.geographies$LEVEL[nucleus::k.geographies$GEOGRAPHY == "Block Group"],
                   GEOID = query,
                   STATE = nucleus::k.states$SHORT[nucleus::k.states$GEOID == substr(query, 0, 2)]))
        } else {
      ...
      if (substr(query, 0, 2) %in% nucleus::k.states$GEOID) {
        t.reference <- suppressMessages(tigris::blocks(state = nucleus::k.states$SHORT[nucleus::k.states$GEOID == substr(query, 0, 2)], year = 2020))
        if (query %in% t.reference$GEOID) {
          return(c(NAME = t.reference$NAME20[t.reference$GEOID == query],
                   GEOGRAPHY = "Block",
                   LEVEL = nucleus::k.geographies$LEVEL[nucleus::k.geographies$GEOGRAPHY == "Block"],
                   GEOID = query,
                   STATE = nucleus::k.states$SHORT[nucleus::k.states$GEOID == substr(query, 0, 2)]))
        } else {
    ...
    if (sum(grepl(paste0("'", query, "'"), toupper(nucleus::k.geographies$ALIAS[nucleus::k.geographies$GEOGRAPHY == "Nation"]))) > 0) {
      return(c(NAME = "Nation",
               GEOGRAPHY = "Nation",
               LEVEL = nucleus::k.geographies$LEVEL[nucleus::k.geographies$GEOGRAPHY == "Nation"],
               GEOID = "US",
               STATE = "No state container"))
      ...
      return(c(NAME = t.reference$NAMELSAD[c(which(toupper(t.reference$NAMELSAD) == query), which(toupper(t.reference$NAME) == query))],
               GEOGRAPHY = "Region",
               LEVEL = nucleus::k.geographies$LEVEL[nucleus::k.geographies$GEOGRAPHY == "Region"],
      ...
      return(c(NAME = t.reference$NAMELSAD[c(which(toupper(t.reference$NAMELSAD) == query), which(toupper(t.reference$NAME) == query))],
               ...
               GEOID = t.reference$GEOID[c(which(toupper(t.reference$NAMELSAD) == query), which(toupper(t.reference$NAME) == query))],
               STATE = "No state container"))
    } else if (query %in% toupper(nucleus::k.states$NAME) | query %in% toupper(nucleus::k.states$SHORT)) {
      return(c(NAME = nucleus::k.states$NAME[c(which(toupper(nucleus::k.states$NAME) == query), which(toupper(nucleus::k.states$SHORT) == query))],
               GEOGRAPHY = "State",
               LEVEL = nucleus::k.geographies$LEVEL[nucleus::k.geographies$GEOGRAPHY == "State"],
